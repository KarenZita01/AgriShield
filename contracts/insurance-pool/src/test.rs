...
fn create_pool(
    ...
    oracle_a: &Address,
    oracle_b: &Address,
) -> (Address, InsurancePoolClient, Address) {
...
fn test_initialize() {
    ...
    let (_, client, _admin) = create_pool(&env, &token_addr, &oracle_a, &oracle_b);
...
fn test_admin_withdraw_before_close() {
    let (env, token_addr, _deployer, oracle_a, oracle_b) = setup();
    let (_, client, admin) = create_pool(&env, &token_addr, &oracle_a, &oracle_b);
    let provider = Address::generate(&env);
    client.deposit_liquidity(&provider, &1_000_000_000i128);
    client.admin_withdraw(&admin, &100_000_000i128);
}
...
fn test_admin_withdraw_after_close() {
    ...
    if balance > 0 {
        client.admin_withdraw(&admin, &balance);
    }
}
...
fn test_full_lifecycle() {
    ...
    // Non-breach readings — pool stays Active
    for i in 1..=3u64 {
